...
export function makeVideoObject (payload, filename) {
  const blob = new Blob([new Uint8Array(payload)], { type: 'video/mp4' })
  return new File([blob], filename)
}

export async function storeFiles (files, isClient) {
...
export async function storeWithProgress (files, isClient) {
...
export function makeFileUrl(cid, fileName) {
...
export async function retrieveFile (url, resType = 'json') {
